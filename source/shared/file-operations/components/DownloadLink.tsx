...
export function DownloadLinkContent({
    ...
    useEffect(() => {
        ...
        const { expiry, password, limit } = meta_data;
    ...
    const handleGenerateLink = async () => {
        ...
        if (meta_data?.key) {
            config.download_link_id = meta_data?.key;
        }
            ...
            if (data?.data) {
                ...
                if (meta_data) {
                    window.dispatchEvent(
                        new CustomEvent("DOWNLOAD_LINK_UPDATED", {
                            detail: {
    ...
    const convertExportOptions = useMemo(() => {
    ...
    return (
        <BlockStack gap={15} padding={0}>
            <InlineStack gap={10} align="between" wrap={false}>
                ...
                >
                    {__("Share Download Link", "ninja-drive")} "{file?.name}"
                </Truncate>
            ...
            <Card background="primary-extralight" flex direction="col" gap={15}>
                ...
                {isExportDocument(file) && (
                    <SelectControl
                        __next40pxDefaultSize
                        label={__("Download Link Format", "ninja-drive")}
                        onChange={(value) => setExportFormat(value)}
                        options={convertExportOptions}
                    />
                )}
                ...
                >
                    {meta_data
                        ? __("Update Download Link", "ninja-drive")
                        : __("Generate Download Link", "ninja-drive")}
                </Button>
                ...
                <DownloadLinkContent
                    ...
                    onConfirm={async (file_key) => {
                        ...
                        showAlert({
                            toast: true,
                            type: "success",
                            text: __("Downloaded successfully", "ninja-drive"),
                            timer: 3000,
                            timerProgressBar: true,
                            showConfirmButton: false,
                        });
