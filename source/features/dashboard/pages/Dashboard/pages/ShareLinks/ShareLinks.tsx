...
const ShareLinks = ({
    ...
    const { shared_files } = useAppSelector(selectDashboard);
    const [files, setFiles] = useState<File[]>(shared_files?.files || []);
    const [filter, setFilter] = useLocalStorage<{
        page: number;
        per_page: number;
    }>("pnpnd-share-files-filter", {
        page: 1,
        per_page: 5,
    });

    const { data, isLoading, isFetching } = useGetSharedFilesQuery(
        {
            page: filter?.page,
    ...
    useEffect(() => {
        ...
        } else if (overview && shared_files?.files) {
            setFiles(shared_files?.files);
        }
    }, [data, shared_files, overview, isLoading, isFetching]);
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    >
                        {overview
                            ? __("Recent Share Links", "ninja-drive")
                            : __("All Share Links", "ninja-drive")}
                    </Text>
                    ...
                    {overview && (
                        <Button
                            ...
                            onClick={() => navigate("/dashboard/share-links")}
                        >
