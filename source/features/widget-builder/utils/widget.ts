...
export const getModuleDocLink = (widget_type: ModuleKey) => {
    const docMap: Record<string, string | undefined> = {
        file_browser: DOCS?.MODULE_BUILDER?.fileBrowser?.link,
        file_list: DOCS?.MODULE_BUILDER?.fileList?.link,
        media_player: DOCS?.MODULE_BUILDER?.mediaPlayer?.link,
        gallery: DOCS?.MODULE_BUILDER?.gallery?.link,
        slider_carousel: DOCS?.MODULE_BUILDER?.sliderCarousel?.link,
        file_uploader: DOCS?.MODULE_BUILDER?.fileUploader?.link,
        embed_documents: DOCS?.MODULE_BUILDER?.embedDocuments?.link,
        search_box: DOCS?.MODULE_BUILDER?.searchBox?.link,
    };

    return docMap[widget_type || ""];
};

export const isProWidget = (widget_type: ModuleKey) => {
    const proWidgets: ModuleKey[] = [
        "file_list",
        "media_player",
        "slider_carousel",
        "file_uploader",
    ...
    return proWidgets?.includes(widget_type);
};
